            ...
            <v-layout row wrap justify-space-between>
                ...
                <v-flex md5 xs12 mb-4>
                    ...
                    <v-form ref="form" lazy-validation>
                        <v-select v-model="service" :items="services" item-text="title" label="Виберіть абонемент" box
                                  required></v-select>
                        <v-text-field v-model="name" label="Ваше ім'я" box required></v-text-field>
                        <v-text-field v-model="email" label="Email(пошта)" box required></v-text-field>
                        <v-dialog ref="datepicker" v-model="datemodal" :return-value.sync="date" persistent lazy
                                  full-width width="290px">
                            <template v-slot:activator="{ on }">
                                <v-text-field v-model="date" label="Дата початку дії абонементу" append-icon="event"
                                              readonly v-on="on" box required></v-text-field>
                            </template>
                        ...
                        <v-dialog ref="timepicker" v-model="timemodal" :return-value.sync="time" persistent lazy
                                  full-width width="290px">
                            <template v-slot:activator="{ on }">
                                <v-text-field v-model="time" label="Час" append-icon="access_time" readonly v-on="on"
                        ...
                        <v-text-field v-model="message" label="Короткий опис та місцезнаходження обладнання"
                                      height="200px" box required></v-text-field>
